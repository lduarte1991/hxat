    ...
    def __init__(self, config=None, lti_params=None):
        ...
        try:
            ...
            self.course_attrs = {
                ...
                "lti_tool_consumer_instance_guid": lti_params[
            ...
            if lti_params.get("lis_course_offering_sourcedid"):
                self.course_attrs["sis_course_id"] = lti_params[
                    "lis_course_offering_sourcedid"
                ]
            if lti_params.get("custom_canvas_course_id"):
                self.course_attrs["canvas_course_id"] = int(
                    lti_params["custom_canvas_course_id"]
                )
        except (KeyError, ValueError) as e:
            raise ImageStoreBackendException(
                "Missing or invalid LTI params: %s" % str(e)
            )
    ...
    def store(self, uploaded_files, title):
        ...
        try:
            ...
            collection = self.client.api.create_collection(
                course_id=course["id"], title=title, description="",
            )
            ...
            logger.info(
                "User %s obtained manifest for collection: %s"
