...
Annotator.Plugin.Grouping = (function(_super) {
    ...
    Grouping.prototype.pluginInit = function() {
        ...
        this.options.optionsOVA.annotation_tool.subscribe('annotationsLoaded', this.reloadAnnotations);
        this.options.optionsOVA.annotation_tool.subscribe('annotationUploaded', this.reloadAnnotations);
        this.options.optionsOVA.annotation_tool.subscribe('annotationDeleted', this.reloadAnnotations);
        this.options.optionsOVA.annotation_tool.subscribe('annotationCreated', this.reloadAnnotations);
        this.options.optionsOVA.annotation_tool.subscribe('changedTabsInCatch', this.groupingButtonPressed);
        ...
        if (this.options.optionsOVA.default_tab.toLowerCase() === 'public') {
            newdiv.innerHTML = "Annotation Grouping: ON";
            this.useGrouping = 1;
            // we wait for HighlightTags to complete before reloading annotations
            //this.options.optionsOVA.annotation_tool.subscribe('colorizeCompleted', this.reloadAnnotations);
        } else {
            newdiv.innerHTML = "Annotation Grouping: OFF";
            $(newdiv).addClass('buttonOff');
            this.useGrouping = 0;
        }
        $($('.annotator-wrapper')[0]).append(newdiv);
    ...
    Grouping.prototype.groupAndColor = function() {
        ...
        annotations.forEach(function(annot) {
            if (annot.highlights !== undefined && annot.highlights.length > 0) {
    ...
    Grouping.prototype.reloadAnnotations = function() {
        var annotations = this.options.optionsOVA.annotation_tool.plugins['Store'].annotations;
        ...
        $.each(this.groupedAnnotations, function(key, val) {
            if (val.length > self.groupthreshold) {
                ...
                $(newdiv).click(function(evt){
                    ...
                    if($(target).attr("data-selected") === '0') {
                        ...
                        self.groupedAnnotations[$(target).css("top").replace("px", "")].forEach(function(item) {
                        ...
                        $('.groupButton').each( function(val, button){
    ...
    Grouping.prototype.groupingButtonPressed = function() {
        if(this.useGrouping === 1) {
            ...
            this.options.optionsOVA.annotation_tool.unsubscribe('annotationsLoaded', this.reloadAnnotations);
            this.options.optionsOVA.annotation_tool.unsubscribe('annotationUploaded', this.reloadAnnotations);
            this.options.optionsOVA.annotation_tool.unsubscribe('annotationDeleted', this.reloadAnnotations);
            this.options.optionsOVA.annotation_tool.unsubscribe('annotationCreated', this.reloadAnnotations);
            
            // redraw button to turn grouping on/off
            $(".onOffGroupButton").html("Annotation Grouping: OFF");
            $(".onOffGroupButton").addClass("buttonOff");
            this.options.optionsOVA.annotation_tool.plugins.Store.annotations.forEach(function(annot) {
                if (annot.highlights !== undefined) {
                    $.each(annot.highlights, function(key, ann) {
            ...
            this.options.optionsOVA.annotation_tool.publish('externalCallToHighlightTags');
            this.options.optionsOVA.annotation_tool.unsubscribe('colorizeCompleted', this.reloadAnnotations);
        } else {
