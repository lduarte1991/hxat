...
class LTILaunchSession(object):
    ...
    def assert_valid(self):
        ...
        if not self.resource_link_id:
            ...
            raise LTILaunchError("Invalid LTI session: resource_link_id %s not in LTI_LAUNCH dict" % self.resource_link_id)
...
class CookielessSessionMiddleware(MiddlewareMixin):
    ...
    def process_request(self, request):
        ...
        if session_key is not None:
            self.logger.info("Session cookie '%s' returned key: %s" % (settings.SESSION_COOKIE_NAME, session_key))
        else:
            self.logger.info("Session cookie '%s' not found!" % settings.SESSION_COOKIE_NAME)
...
class MultiLTILaunchMiddleware(MiddlewareMixin):
    ...
    def _update_session(self, request):
        ...
        self.logger.info("LTI launch sessions: %s [max=%s]" % (lti_launches.keys(), max_launches))
        ...
        self.logger.info("LTI launch session saved: resource_link_id={resource_link_id} user_id={user_id}".format(user_id=user_id, resource_link_id=resource_link_id))
