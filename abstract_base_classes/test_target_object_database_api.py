...
class LTITODAPITests(TestCase):
	...
	def test_abc_TODAPI_LTI(self):
		...
		# not sure if this is considered cheating, but as we are testing the non-implementation
		# of functions, this might be okay. Because you can't make abstract methods static within
		# an abstract base class, it treats them as class methods, thus below we are passing in
		# one less trivial parameter because class methods also pass in "self" as a parameter first
		self.assertRaises(NotImplementedError, self.sampleTODAPI.get_own_targets_from_course, '')
		self.assertRaises(NotImplementedError, self.sampleTODAPI.get_own_targets_as_user)
...
class TOD_ImplementationTests(TestCase):
	...
	def setUp(self):
		...
		self.ltiprofile1 = LTIProfile.objects.get(user=self.createFakeUser("fakeusername1", "fakeuserid1"))
		self.ltiprofile2 = LTIProfile.objects.get(user=self.createFakeUser("fakeusername2", "fakeuserid2"))
		...
		self.target1 = TargetObject(target_title="TObj1", target_author="Test Author", target_content="Fake Content", target_citation="Fake Citation", target_type="tx")
		self.target1.save()
		self.target1.target_courses.add(self.samplecourse)
		self.target2 = TargetObject(target_title="TObj2", target_author="Test Author", target_content="Fake Content2", target_citation="Fake Citation2", target_type="tx")
		self.target2.save()
	...
	def test_get_own_targets_from_course(self):
		"""
		"""
		self.assertTrue(len(TOD_Implementation.get_own_targets_from_course(self.ltiprofile1, 'fake_course'))==2)
		self.assertRaises(ObjectDoesNotExist, TOD_Implementation.get_own_targets_from_course, self.ltiprofile1, 'fake_course2')
		course2 = LTICourse.create_course('fake_course2', self.ltiprofile1)
		self.assertTrue(len(TOD_Implementation.get_own_targets_from_course(self.ltiprofile1, 'fake_course2'))==0)

	def test_get_dict_of_files_from_courses(self):
		"""
		"""
		dict_of_targets = TOD_Implementation.get_dict_of_files_from_courses(self.ltiprofile1, LTICourse.get_all_courses())
	...
	def test_get_own_targets_as_user(self):
		"""
		"""
		self.assertTrue(len(TOD_Implementation.get_own_targets_as_user(self.ltiprofile1))==2)
		self.assertTrue(len(TOD_Implementation.get_own_targets_as_user(self.ltiprofile2))==0)
