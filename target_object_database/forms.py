...
class SourceForm(forms.ModelForm):
        ...
        if target_type in ("tx", "vd"):
            ...
            if settings.IMAGE_STORE_BACKEND:
                if not self.files and not target_content:
                    self.add_error(
                        None,
                        "Please choose an image to upload OR provide a source manifest URL.",
                    )
    ...
    def validate_manifest_unique(self, target_content):
        ...
        if found and found.pk != self.instance.pk:
            msg = (
                "The image manifest must be unique. It is already provided by source '%s' (ID:%s)."
                % (found.target_title, found.pk)
            )
    ...
    def save(self):
        if settings.IMAGE_STORE_BACKEND and self.files:
            try:
                self.instance.target_content = handle_file_upload(
                    self.data, self.files, self.request.LTI["launch_params"]
                )
            except ValidationError as e:
...
def handle_file_upload(data, files, lti_params):
    ...
    try:
        ...
        image_backend = cls(
            config=settings.IMAGE_STORE_BACKEND_CONFIG, lti_params=lti_params
        )
        manifest_url = image_backend.store(
            [file for file in list_of_files], title
        )  # Intentionally passed as a list() instead of MultiValueDict
    except image_store.backends.ImageStoreBackendException as e:
        raise ValidationError("Error uploading image. Details: %s" % e)

    if not manifest_url:
        raise ValidationError(
            "Failed to get manifest after uploading image (manifest required for annotation)."
        )
